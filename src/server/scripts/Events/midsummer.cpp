...
struct npc_midsummer_bonfire : public ScriptedAI
{
    ...
    bool InitBonfire()
    {
        ...
        if ((itr != BonfireStateStore.end()) && (itr->second))
            ...
            TC_LOG_ERROR("scripts.midsummer", "NPC {} (GUID{}) in map {}, zone {} with teamId {} can't locate its entry within BonfireStateStore", me->GetGUID().GetEntry(), me->GetSpawnId(), me->GetMapId(), me->GetZoneId(), _teamId);
...
struct npc_midsummer_ribbon_pole_target : public ScriptedAI
{
    ...
    void SpawnFireSpiralBunny()
    {
        ...
        if (!_bunny)
            _bunny = DoSpawnCreature(NPC_RIBBON_POLE_FIRE_SPIRAL_BUNNY, 0, 0, 0, 0, TEMPSUMMON_MANUAL_DESPAWN, 0ms);
    }
    ...
    void DoSpewLavaChecks()
    {
        ...
        if (_dancerList.size() >= THRESHOLD_SPEW_LAVA)
        {
            if (!_dancerList.empty())
            {
                Trinity::Containers::RandomShuffle(_dancerList);
                ...
                for (uint8 i = 0; (i < MAX_COUNT_SPEW_LAVA_TARGETS) && (i < _dancerList.size()); i++)
                {
                    ...
                    {
                        TempSummon* fireSpiralBunny = dancerTarget->SummonCreature(NPC_RIBBON_POLE_FIRE_SPIRAL_BUNNY, dancerTarget->GetPositionX(), dancerTarget->GetPositionY(), dancerTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 10s);
                        if (fireSpiralBunny)
                        {
    ...
    void DoDancingFLameChecks()
    {
        if (_dancerList.size() >= THRESHOLD_DANCING_FLAMES)
        {
            ...
            if (crList.size() < MAX_COUNT_DANCING_FLAMES)
            {
                ...
                DoSpawnCreature(NPC_BIG_DANCING_FLAMES, spawnDist * cos(angle), spawnDist * std::sin(angle), 0, angle + M_PI, TEMPSUMMON_TIMED_DESPAWN, 60s);
            }
        }
    }
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Unit* target = GetTarget())
        {
            ...
            if ((GameTime::GetGameTime() - GetApplyTime()) > 60 && target->GetTypeId() == TYPEID_PLAYER)
                target->ToPlayer()->UpdateCriteria(CriteriaType::BeSpellTarget, 58934, 0, 0, target);
        }
    }
...
class spell_midsummer_torch_quest : public AuraScript
{
    ...
    {
        Unit* ar = GetTarget();
        if (Creature* cr = ar->SummonCreature(NPC_TORCH_TOSS_TARGET_BUNNY, ar->GetPositionX(), ar->GetPositionY(), ar->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN, 90s))
        {
...
class spell_midsummer_fling_torch : public SpellScript
{
    ...
    void ThrowNextTorch(Unit* caster)
    {
        ...
        if (GetSpellInfo()->Id == SPELL_FLING_TORCH_DUMMY)
            angle = frand(-float(M_PI), float(M_PI)); // full circle
        else
...
class spell_midsummer_summon_ahune_lieutenant : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (npcEntry)
            caster->SummonCreature(npcEntry, caster->GetPosition(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 1min);
    }
