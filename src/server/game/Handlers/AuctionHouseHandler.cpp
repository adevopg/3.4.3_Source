...
void WorldSession::HandleAuctionBrowseQuery(WorldPackets::AuctionHouse::AuctionBrowseQuery& browseQuery)
{
    ...
    auctionHouse->BuildListBuckets(result, _player, name, browseQuery.MinLevel, browseQuery.MaxLevel, 
                                   browseQuery.Filters, classFilters, browseQuery.KnownPets, browseQuery.MaxPetLevel, 
                                   browseQuery.Offset, browseQuery.Sorts);
...
{
    AuctionThrottleResult throttle = sAuctionMgr->CheckThrottle(_player, /*listBiddedItems.TaintedBy.has_value()*/false); // alistar:
    if (throttle.Throttled)
        return;
...
void WorldSession::HandleAuctionListPendingSales(WorldPackets::AuctionHouse::AuctionListPendingSales&)
{
    WorldPackets::AuctionHouse::AuctionListPendingSalesResult response;

    for (Mail* m : _player->GetMails())
    {
        // skip non auction mails and mails that have been delivered
        if (m->messageType != MailMessageType::MAIL_AUCTION || GameTime::GetGameTime() > m->deliver_time)
            continue;

        if (response.Mails.size() < size_t(AuctionHouseResultLimits::Items))
            response.Mails.emplace_back(m, _player);
...
void WorldSession::HandleAuctionPlaceBid(WorldPackets::AuctionHouse::AuctionPlaceBid& placeBid)
{
    ...
    if (!auction /*|| auction->IsCommodity()*/) // alistar: TODO
    {
    ...
    else
    {
        ...
        // alistar: add the auction to the bidderList so it can be displayed on the Bid UI
        auto& playerBidderList = auctionHouse->GetPlayerBidderAuctions();

        bool auctionExists = false;

        for (const auto& [guid, auctionId] : playerBidderList)
        {
        ...
        if (!auctionExists)
            playerBidderList.insert({ player->GetGUID(), auction->Id });
        ...
        [this, auctionId = placeBid.AuctionID, bidAmount = placeBid.BidAmount, auctionPlayerGuid = _player->GetGUID(), throttle](bool success)
    {
        if (GetPlayer() && GetPlayer()->GetGUID() == auctionPlayerGuid)
        {
            if (success)
            {
                ...
                // alistar: wave after successful bid
                GetPlayer()->HandleEmoteCommand(EMOTE_ONESHOT_WAVE);
            }
            else
...
{
    AuctionThrottleResult throttle = sAuctionMgr->CheckThrottle(_player, false, AuctionCommand::Cancel);
    if (throttle.Throttled)
        return;
...
void WorldSession::SendAuctionHello(ObjectGuid guid, Creature* unit)
{
    ...
    auctionHelloResponse.PurchasedItemDeliveryDelay = ahEntry->DepositRate;
    auctionHelloResponse.CancelledItemDeliveryDelay = ahEntry->ConsignmentRate;
