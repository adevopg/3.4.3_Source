...
public:
    ...
    bool Initialize(uint32 accountId, bool withDeclinedNames, bool isDeletedCharacters)
    {
        ...
        bool result = true;
        CharacterDatabaseStatements statement = statements[isDeletedCharacters ? 1 : 0][withDeclinedNames ? 1 : 0];
        
        CharacterDatabasePreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(statement);
        if (statement == CHAR_SEL_ENUM || statement == CHAR_SEL_ENUM_DECLINED_NAME ||
            statement == CHAR_SEL_UNDELETE_ENUM || statement == CHAR_SEL_UNDELETE_ENUM_DECLINED_NAME)
        {
            stmt->setUInt32(0, PET_SAVE_AS_CURRENT);
            stmt->setUInt32(1, PET_SAVE_NOT_IN_SLOT);
...
void WorldSession::HandleCharCreateOpcode(WorldPackets::Character::CreateCharacter& charCreate)
{
    ...
    if (charCreate.CreateInfo->Class == CLASS_DEATH_KNIGHT && !HasPermission(rbac::RBAC_PERM_SKIP_CHECK_CHARACTER_CREATION_DEATH_KNIGHT))
    {
        ...
        {
            SendCharCreate(CHAR_CREATE_DEATHKNIGHT_DUPLICATE);
            return;
        }
        ...
        {
            SendCharCreate(CHAR_CREATE_DEATHKNIGHT_LEVEL_REQUIREMENT);
            return;
        }
    }
        ...
        .WithChainingPreparedCallback([this, createInfo](QueryCallback& queryCallback, PreparedQueryResult result)
    {
        ...
        stmt->setUInt32(0, GetAccountId());
        stmt->setUInt32(1, (skipCinematics == 1 || createInfo->Class == CLASS_DEATH_KNIGHT) ? 1200 : 1); // 200 (max chars per realm) + 1000 (max deleted chars per realm)
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    if (pCurrChar->m_deathState == DEAD)
    {
        ...
        // alistar: send resurrect timer on login
...
{
    if (packet.ShowHelm)
        _player->RemovePlayerFlagEx(PLAYER_FLAGS_EX_HIDE_HELM);
    else
        _player->SetPlayerFlagEx(PLAYER_FLAGS_EX_HIDE_HELM);
}

void WorldSession::HandleShowingCloakOpcode(WorldPackets::Character::ShowingCloak& packet)
{
    if (packet.ShowCloak)
        _player->RemovePlayerFlagEx(PLAYER_FLAGS_EX_HIDE_CLOAK);
    else
        _player->SetPlayerFlagEx(PLAYER_FLAGS_EX_HIDE_CLOAK);
}
