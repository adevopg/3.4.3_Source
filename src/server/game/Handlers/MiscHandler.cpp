...
void WorldSession::HandleSetPvP(WorldPackets::Misc::SetPvP& packet)
{
    ...
    else
    {
        ...
        // PvP Timer
        if (player->HasPlayerFlag(PLAYER_FLAGS_PVP_TIMER))
            player->RemovePlayerFlag(PLAYER_FLAGS_PVP_TIMER);
        else
...
{
    // alistar: interrupt auto shot after changing target
    if (_player->m_unitData->Target != packet.Selection)
    {
        if (_player->m_currentSpells[CURRENT_AUTOREPEAT_SPELL] &&
            _player->m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->GetSpellInfo()->Id == 75)
        {
...
void WorldSession::HandleCloseInteraction(WorldPackets::Misc::CloseInteraction& closeInteraction)
{
    ...
    // nothing will happen because there's a cheating check if interaction data matches source
    // when we close interaction we reset that interaction and we won't be able to use that option
    // In my opinion this is not as great but it's fine since when we talk to a npc the first thing we do
    // is reset the interaction data.
