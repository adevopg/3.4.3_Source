...
BattlePayDataStoreMgr::BattlePayDataStoreMgr()
    : _purchaseIDCount(1) { }
...
void BattlePayDataStoreMgr::LoadDisplayInfos()
{
    ...
    do
    {
        ...
        if (const uint32 hasCreatureDisplayId = fields[1].GetUInt32())
            displayInfo.CreatureDisplayID = hasCreatureDisplayId;

        if (const uint32 hasVisualId = fields[2].GetUInt32())
            displayInfo.VisualID = hasVisualId;
        ...
        if (const uint32 hasFlags = fields[10].GetUInt32())
            displayInfo.Flags = hasFlags;

        if (const uint32 hasUnk1 = fields[11].GetUInt32())
            displayInfo.Unk1 = hasUnk1;

        if (const uint32 hasUnk2 = fields[12].GetUInt32())
            displayInfo.Unk2 = hasUnk2;

        if (const uint32 hasUnk3 = fields[13].GetUInt32())
            displayInfo.Unk3 = hasUnk3;
...
void BattlePayDataStoreMgr::InitMaxPurchaseId()
{
    auto result = LoginDatabase.PQuery("SELECT MAX(`id`) + 1 FROM battlepay_purchases;");
    if (!result)
        return;

    auto id = result->Fetch()[0].GetUInt32();
    _purchaseIDCount = id;
}
...
void BattlePayDataStoreMgr::LoadProduct()
{
    ...
    do
    {
        ...
        if (const uint32 UnkInts = fields[7].GetUInt32())
            productInfo.UnkInts.push_back(UnkInts);
    ...
    do
    {
        ...
        const std::string UnkString = fields[13].GetString();
        if (!UnkString.empty() && UnkString[0] != '0')
            product.UnkString = UnkString;

        product.UnkBit = fields[14].GetUInt32();

        if (const uint32 UnkBits = fields[15].GetUInt32())
...
uint64 BattlePayDataStoreMgr::GenerateNewPurchaseID()
{
    std::lock_guard<std::mutex> lock(_Mu);

    if (_purchaseIDCount >= 0xFFFFFFFE)
    {
        TC_LOG_ERROR("server.battlepay", "_purchaseIDCount ids overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return _purchaseIDCount++;
}
...
std::vector<BattlePayData::Product> const* BattlePayDataStoreMgr::GetProductsOfProductInfo(uint32 productInfoEntry)
{
    ...
    TC_LOG_DEBUG("server.BattlePay", "GetProductsOfProductInfo failed for productInfoEntry {}", productInfoEntry);
    return nullptr;
}
...
BattlePayData::ProductInfo const* BattlePayDataStoreMgr::GetProductInfoForProduct(uint32 productID)
{
    ...
    {
        for (const auto& productInfo : _productInfos)
        {
            if (productInfo.second.ProductId == productID)
            {
                TC_LOG_DEBUG("server.battlepay", "GetProductInfoForProduct succeed {}", productID);
                return &_productInfos.at(productInfo.first);
            }
        }

        TC_LOG_DEBUG("server.BattlePay", "GetProductInfoForProduct failed for productID {}", productID);
        return nullptr;
    }
...
std::vector<BattlePayData::ProductItem> const* BattlePayDataStoreMgr::GetItemsOfProduct(uint32 productID)
{
    ...
    TC_LOG_DEBUG("server.BattlePay", "GetItemsOfProduct failed for productid {}", productID);
    return nullptr;
}
